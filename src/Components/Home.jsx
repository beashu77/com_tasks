...
const Home = () => {
    ...
    },
    {
      ...
      ratings: [
        ...
        },
        {
          ...
          pp: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSoZ_Xg9XbpVTth91jSsUrrGzlx_vYZ8pD4YLfGUR4CAdzq_D-DhN_&s=0",
          revName: "Kripla",
        },
      ],
      averageRating: 3.3,
  ...
  const handleClickItalian = (e) => {
    let currData = restaurantsData.filter((elem) => elem.cuisine_id === 1);
    setData(currData);
    console.log(currData);
  };
  const handleClickMexican = (e) => {
    let currData = restaurantsData.filter((elem) => elem.cuisine_id === 2);
    setData(currData);
  };
  const handleClickChinese = (e) => {
    let currData = restaurantsData.filter((elem) => elem.cuisine_id === 3);
    setData(currData);
  };
  const handleClickIndian = (e) => {
    let currData = restaurantsData.filter((elem) => elem.cuisine_id === 4);
    setData(currData);
  };
  ...
  return (
    <div id="mainContainer">
      ...
      <button id="red" onClick={() => handleClickItalian("Italian")}>Italian</button>
      <button id="red" onClick={() => handleClickMexican("Mexican")}>Mexican</button>
      <button id="red" onClick={() => handleClickChinese("Chinese")}>Chinese</button>
      <button id="red" onClick={() => handleClickIndian("Indian")}>Indian</button>

      <div id="outerContainer">
        {data &&
          data.map((elem) => {
            <h2>{elem.name}</h2>
            return <div id="restContainer">
                
               {elem.menu.map((el)=>{
                return <div id="card" onClick={()=>{navigate(`/restaurent/${elem.id}`)}}>
